...
const PACKAGES = {
  "linux x64": "@six5536/superdev-linux-x64",
  "linux arm64": "@six5536/superdev-linux-arm64",
  "darwin x64": "@six5536/superdev-darwin-x64",
  "darwin arm64": "@six5536/superdev-darwin-arm64",
  "win32 x64": "@six5536/superdev-win32-x64",
};
...
function binaryName(platform) {
  return platform === "win32" ? "superdev.exe" : "superdev";
}
...
function resolveBinary(platform, arch, requireResolve = require.resolve) {
  ...
  if (!pkg) {
    throw new Error(
      `No prebuilt superdev binary for ${platform}-${arch}.\n` +
        `Supported platforms: ${SUPPORTED}.\n` +
        `Install from source instead: cargo install superdev`,
    );
  }
  try {
    ...
    throw new Error(
      `The superdev platform package "${pkg}" is not installed.\n` +
        `Optional dependencies were most likely skipped during install.\n` +
        `Reinstall superdev, or build from source: cargo install superdev`,
    );
  }
}
