...
pub fn check_unit(
    ...
    let k = &g.kinds.unit;
    let lines: Vec<&str> = crate::fsutil::lines(text);
...
pub fn schema_comparables(file: &str, text: &str, g: &Grammar) -> Vec<Comparable> {
    ...
    let all: Vec<&str> = crate::fsutil::lines(&fences[0].text);
...
mod schema_parity {
    ...
    fn every_shipped_schema_passes() {
        ...
        // 38 since the three tracker schemas folded into one (I052, ADR-050).
