...
pub fn validate_repo(
    ...
    if !schema_files.is_empty() {
        ...
        // A variant declaration nothing reads (ADR-045), an `item-key` whose
        // capture count names no key (ADR-047), and each schema's example
        // against the schema declaring it, in place — findings land on the
        // schema file (ADR-024).
        schema_findings.extend(schema::document::check_variants(&schema_files));
        schema_findings.extend(schema::document::check_item_keys(&schema_files));
...
mod tests {
    ...
    fn this_repository_checks_its_documents_against_its_schemas() {
        let root = repo();
        let run = validate_repo(&root, &root.join("knowledge"), &[], &live()).unwrap();
        assert!(run.schemas >= 37, "schemas read: {}", run.schemas);
