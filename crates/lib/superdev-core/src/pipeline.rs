...
/// Agent instructions are authored as Markdown; Rust only expands the one
/// capability-dependent region.
const AGGREGATOR_TEMPLATE: &str = include_str!("agent-instructions.md");
const CODE_INDEX_START: &str = "{{superdev:code-index}}\n";
const CODE_INDEX_END: &str = "{{/superdev:code-index}}\n";

/// The fenced aggregator: superdev's agent instructions as one document.
fn aggregator_content(manifest: &Manifest) -> String {
    let (before, rest) = AGGREGATOR_TEMPLATE
        .split_once(CODE_INDEX_START)
        .expect("agent instructions carry the code-index start marker");
    let (code_index, after) = rest
        .split_once(CODE_INDEX_END)
        .expect("agent instructions carry the code-index end marker");
    if manifest.enabled(Capability::CodeIndex) {
        format!("{before}{code_index}{after}")
    } else {
        format!("{before}{after}")
    }
}
...
mod tests {
    ...
    fn aggregator_imports_track_the_enabled_set() {
        ...
        assert!(
            !all.contains("<tool_call name=\"sokf_overview\" when=\"always\""),
            "{all}"
        );
        assert!(
            !all.contains("path=\".agents/sokf/SPEC.md\" when=\"always\""),
            "{all}"
        );
        assert!(!all.contains("@../knowledge/index.md"), "{all}");
        assert!(!all.contains("{{superdev:"), "{all}");
