#!/usr/bin/env node
// Cut a release commit and tag. Deliberately stops before pushing: the push
// is what triggers an irreversible publish, so it stays a human action.
//
...
//   5. commit and tag
//
// Checks 1 and 2 run before anything is written, so a failure leaves the
// tree untouched.
...
// The gate that makes "update the changelog" non-optional; the release
...
Pushing the tag triggers the release workflow, which publishes to npm.
Published versions cannot be fully undone.`);
