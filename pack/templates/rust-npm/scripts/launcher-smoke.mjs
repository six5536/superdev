...
const { selectPackage, binaryName } = require_(`../packages/${NAME}/lib/binary.js`);
...
const base = mkdtempSync(join(tmpdir(), `${NAME}-launcher-smoke-`));
try {
  // Pack both packages: what npm would publish, `files` manifests applied.
  const pack = run("npm", ["pack", `./packages/${NAME}`, `./${pkgDir}`, "--pack-destination", base]);
  if (pack.status !== 0) {
  ...
  const dests = [join(nm, NAME), join(nm, pkgName)];
  ...
  const launcher = join(nm, NAME, "bin", `${NAME}.js`);
  const version = run(process.execPath, [launcher, "--version"]);
  if (
    version.status !== 0 ||
    !new RegExp(`^${NAME} \\d+\\.\\d+\\.\\d+`).test(version.stdout.trim())
  ) {
