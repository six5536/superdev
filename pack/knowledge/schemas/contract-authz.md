...
description: What a caller may do — the principals, the role and scope vocabulary, the permissions and the boundaries every surface enforces, a public contract.
---

# Authorisation Contract Schema

Structural rules for one public authorisation contract, filed at
`contract-{nnn}-authz-{slug}`, a public contract placed in its lifecycle folder by `superdev validate --fix`. The role and scope vocabulary, what each may do,
