...
example: |
  ...
  - Filed as `issue-{nnn}-{slug}`, one issue per reproducible symptom,
    with `superdev validate --fix` placing the file by its `lifecycle`.
    Two symptoms with one cause stay two issues until the cause is proven.
  - A resolved issue is never deleted. It stays, its `lifecycle` set to
    `done` or `wontfix`, so the search that found it once finds the answer
    next time.
