...
<step name="PRESENT THE CHANGE SET" task="Present the complete change set to the user — every contract and ADR created or changed, what each change binds, and the rejected alternatives — and ask for the go-ahead; the presentation is an interaction, never a self-check, and nothing commits before it" />
...
<step name="COMMIT THE CONTRACTS" task="Commit the approved contract and decision-record edits on the feature's branch, per the `development-procedure` concept's commit convention" />
