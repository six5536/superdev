...
3. Preserve unrelated staged, unstaged, and untracked work. If another writer is using the destination, wait rather than race it. Do not stash, reset, force a checkout, overwrite pending files, merge product work, or push.
...
3. Before writing, refuse pre-existing edits to the destination index or record path, including staged, unstaged, untracked, and ignored files. Run read-only `superdev validate` first; if unrelated knowledge needs repair, stop without changing it. Write the schema-conforming record in the destination's canonical knowledge and add its index entry. Preserve the user's meaning and relevant links. Use the SOKF-authoring rules and safe mutation tools; when working in another worktree, run the ordinary SOKF CLI from that worktree so mutations target its knowledge, not the caller's.
4. Run `superdev validate --fix` in the destination until PASS. Inspect every repair and the diff; do not commit repairs to unrelated documents. If validation requires unrelated changes or a concurrent writer has taken the chosen ID, stop and reassess rather than overwriting work.
