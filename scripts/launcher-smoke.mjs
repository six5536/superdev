...
const { selectPackage, binaryName } = require_("../packages/superdev/lib/binary.js");
...
const base = mkdtempSync(join(tmpdir(), "superdev-launcher-smoke-"));
try {
  // Pack both packages: what npm would publish, `files` manifests applied.
  const pack = run("npm", ["pack", "./packages/superdev", `./${pkgDir}`, "--pack-destination", base]);
  if (pack.status !== 0) {
