...
You decide only the interfaces that will be expensive to change once other code depends on them. Design the feature's interfaces as specified in the input above, following `schema-contract-interface` and `schema-adr`.
</goal>

<bootstrap_actions>
<tool_call name="read_file" path=".agents/core.md" when="always" />
<tool_call name="read_file" path="knowledge/schemas/contract-interface.md" when="always" />
...
<tool_call name="sokf_search" query="the public contracts this project offers" when="always" />
...
<step name="WRITE BACKEND CONTRACTS" task="Backend interfaces: a written contract per `schema-contract-interface`" />
