...
- Support skills the phases call: `/grill-me` (interview the user until one reading remains) and `/double-check` (check the last work) in every document phase; `/brainstorm` from `/frame`, to widen an idea too unshaped to state a goal; `/prototype` from `/interface-design`, for throwaway code answering a question conversation cannot settle; `/research` from `/frame` and `/interface-design`, for external facts from primary sources, filed in the bundle. All four also run standalone.
- Knowledge upkeep: `/bootstrap` (fill the bundle from the repo and the owner; `/frame` calls it when an existing project's bundle is empty), `/maintain` (audit and repair the bundle and the workflow's records; run regularly).</step>
