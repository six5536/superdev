...
- Tests: paired creation, overwrite, nested-parent, error, alias-queue, pre-abort, in-flight abort, policy-rejection, repair, and refiling cases cover `contract-012-api-sokf-pi-file-tools P_write-parity`, `P_agent-safe-routing`, `P_context-shape`, and `contract-003-api-sokf P_mutation-diagnostics-bounded`. An applied-then-aborted case proves final validation still runs. Session cases prove every complete lifecycle entry and validation follow-up, including truncation guidance, stops at 200 lines or 8 KiB and directs truncated validation output to `superdev validate`.
