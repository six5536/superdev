...
The functional requirements, each naming the outcome it serves:

- FR-1 — The binary validates unit, schema and core files, choosing the kind
  by the grammar's own `match` rules including `except` (O1).
- FR-2 — For any file, the finding texts the binary emits equal those the
  Node script emits (O1).
- FR-3 — The grammar is read from YAML at run time into types that reject
  unknown keys (O2).
- FR-4 — A grammar that violates its own constraints fails the run before any
  file is read, naming the offending key (O2).
- FR-5 — One command emits one report covering the AOKF checks and the format
  checks, with one `--json` shape and one exit code (O4).
- FR-6 — Given no paths, the command checks the canonical knowledge and the
  trees the grammar's `roots` names; a positional path overrides both (O4).
- FR-7 — One PostToolUse hook runs that same whole-set check on an edit under
  the canonical knowledge or any root, so hook and CI verdicts cannot differ
  (O4).
- FR-8 — A format finding fails the run without altering any AOKF finding
  (O4).
- FR-9 — The command renders the grammar as prose, as the reference's `--doc`
  does today (O1).
- FR-10 — `scripts/superdev-format/` no longer exists (O3).
- FR-11 — With no `.agents/format/grammar.yaml` present, the binary validates
  from its embedded copy (O3).

The non-functional constraints and their budgets:

- NFR-1 — A whole-set run stays fast enough to sit in an edit-time hook
  unnoticed: under 50 ms for the canonical knowledge and the 61 format files
  together, against Node's measured 123 ms for the format half alone.
- NFR-2 — The port adds no runtime dependency beyond the regex engine the
  dialect already assumes: one new crate.
- NFR-3 — The new code clears the repo's coverage gate: 90% lines in each of
  `crates/lib` and `crates/app`.
- NFR-4 — `aokf::validate` keeps emitting exactly what it emits today:
  `validator_parity` passes with its goldens untouched.

Decisions the plan reversed or made moot before the work, kept for the
record; the decisions each block acted on sit with the block:

- D-1 — A new `superdev format validate` verb, over folding it into
  `superdev aokf validate` — reversed by D-17. It read as two specs over two
  trees; the reversal is that they are two checks over one repo, and
  separating them buys nothing while costing a second report, a second hook
  and a second file walk.
- D-15 — A second hook entry beside the aokf one, over one combined hook —
  reversed by D-17. It solved marker churn, which was the wrong problem; with
  one report there is one hook and the existing marker is reused unchanged.
- D-16 — The aokf component owns a second hook entry, over a new `format`
  component — moot under D-17. No second entry is created, so nothing new
  needs an owner; the component-model rethink is unaffected. D-16 is left on
  the record rather than deleted because its reasoning still applies the
  moment anything else needs an owner: the component model wants rethinking
  as a whole, and the components already depend on each other.
- D-19 — Format findings carry an error severity but are excluded from
  `achieved_level`, over letting them grade the ladder like AOKF findings —
  moot under ADR-017. There is no `achieved_level` to keep them out of; a
  format finding is simply fatal, which is what this decision was reaching
  for.
...
- Applying schemas to the documents they govern. `target-files` stays
  ...
  grammar changes block 1 settles. That work is filed as issue-011.
...
- Risk: the finding texts are the contract, and several were reworded during
  ...
  carry reasoning in prose. Mitigation: the goldens fix them in block 2,
  before any wording can drift.
- Risk: the merge reaches into `validate` by accident during block 5 and
  breaks the AOKF goldens, which cannot be regenerated. Early signal:
  `validator_parity` is in the same `cargo test` run as `format_parity`, so
  it fails in block 5 rather than after the reference is gone.
- Risk: JavaScript and Rust regex disagree on a pattern that currently passes,
  ...
  fails in block 4 rather than after the script is gone.
- Risk: the duplication check is order- and tokenisation-sensitive, so a
  ...
  block 4, with its own fixtures, and it is the one place a golden diff is
  expected to need reading rather than accepting.
...
- Risk: the 90% coverage gate lands hardest on the CLI glue. Early signal:
  `npm run coverage:check` fails in block 5, not at the end. `main.rs` already
  uses `#[cfg_attr(coverage_nightly, coverage(off))]` for exactly this.

Notes recorded out of band:
...
would claim. The `roots` list from block 1 excludes them, but they are the
reason a bare run cannot simply walk the repository.

What blocks 5 and 6 settled that the plan left open, recorded here because
each is a decision the decision list did not anticipate.
...
- Change:
  ...
  3. Prove it against the live tree — the Node script still reports 61 passes
     ...
     given paths.
  Files: `scripts/superdev-format/superdev-grammar.yaml` (ledger kind out,
  `roots` in); `scripts/superdev-format/superdev-grammar.meta.yaml` (the same
  two changes); `scripts/superdev-format/validate-superdev.mjs` (drop
  `checkLedger`, read `roots`).
  Decision: D-9 — the `ledger` kind leaves the grammar and the port, over
  keeping it dormant against a future conversion; the awa conversion is a
  finished one-off, a kind nothing writes still costs fixtures, goldens and
  90% coverage, and the script survives in git history if it is ever wanted.
  Decision: D-10 — the grammar names the roots it governs, over hardcoding
  them in the CLI as `aokf_cli.rs` does for `knowledge`; the binary runs in
...
- Change:
  ...
  2. Build the fixture set — one small tree per failure class the script can
     ...
     unknown key, a three-backtick fence, a core file, a duplication hit, and
     a doc render. The classes come from reading the remaining 28 functions.
  3. Capture the goldens — run the reference over each fixture and record the
     output. Hard to reverse: once the script goes in block 6 these cannot be
     regenerated, which is the exact position the AOKF port left itself in.
  Files: `scripts/superdev-format/validate-superdev.mjs` (add `--json`);
  `crates/lib/superdev-core/tests/fixtures/format/` (one fixture tree and
  golden per failure class); `crates/lib/superdev-core/tests/format_parity.rs`
  (the fixtures against the captured goldens).
  Decision: D-4 — parity is proved against goldens captured from the Node
  script, over porting it and reading the diffs by eye; the AOKF port left
  goldens nothing can regenerate, and this reference still runs, so capture
  while it does.
  Decision: D-12 — `--json` is added to the reference first, and the goldens
  are JSON, over capturing the text output as `.golden.txt`; one artifact
  then pins both FR-2 and FR-5, and text goldens would also pin layout, so
  re-ordering two lines would break parity.
...
- Change:
  ...
  Files: `crates/lib/superdev-core/src/format/grammar.rs` (the grammar
  types, `deny_unknown_fields`); `crates/lib/superdev-core/src/format/mod.rs`
  (the module's public surface); `crates/lib/superdev-core/src/lib.rs`
  (declare the module).
  Decision: D-2 — the grammar stays YAML read at run time, over generating
  Rust constants from it at build time; the agent and the user both edit it,
  and a build step puts every grammar change behind a rebuild.
  Decision: D-3 — the Rust types replace the meta-schema, over porting
...
- Change:
  1. Port the readers — `fenceMap`, `splitFrontmatter`, `extractYaml`,
     ...
     reading comparables from unfenced rather than code-stripped text.
  2. Port the per-kind checks — `checkUnit`, `checkSchema`, `checkCore`, each
     against its fixture goldens from block 2.
  3. Port the cross-file checks — duplication by token-set containment, and
     the core-block reference check that runs over units and schemas both.
  Files: `crates/lib/superdev-core/src/format/read.rs` (fences,
...
- Change:
  1. Emit AOKF findings — have the format checks build `Finding` values with
     ...
     per D-18.
  2. Add the merging caller — one function that loads the canonical
     knowledge once, runs `aokf::validate` and the format checks over the
     roots, and returns a single `Report` with findings in file order.
  ...
  Files: `crates/lib/superdev-core/src/format/mod.rs` (the merging caller);
  `crates/lib/superdev-core/src/format/doc.rs` (the grammar rendered as
  prose); `crates/lib/superdev-core/src/format/re.rs` (the grammar's
  patterns, compiled once); `crates/lib/superdev-core/src/format/grammar.yaml`
  (the embedded copy D-6 calls for);
  `crates/lib/superdev-core/src/aokf/validate.rs` (`Finding` and `Report`
  are reused as they are; `validate` untouched);
  `crates/app/superdev/src/main.rs` (a top-level `Validate` arm, `aokf
  validate` hidden); `crates/app/superdev/src/aokf_cli.rs` (the verb runs
  both checks).
  Decision: D-6 — the grammar lives at `.agents/format/grammar.yaml`, read
  at run time, with an embedded copy as the fallback, over `include_str!`
  alone or reading from the resolved pack; it is the same shape
  `.agents/aokf/SPEC.md` already has — a capability's spec beside its
  instruction file — so the agent edits it without a toolchain, and the
  fallback keeps a pack-less repo able to validate.
  Decision: D-7 — format findings are pass or fail, over giving them a
  conformance ladder of their own; levels let a repo adopt the format
  gradually, which is a feature and not a port, and ADR-017 has since
  removed AOKF's ladder too, so there is now one model.
  Decision: D-13 — the doc renderer is ported, over dropping it as dead
  code; deleting the reference is one-way, and a renderer's golden is one
  snapshot rather than a class, so goldens cannot stand in for it.
  Decision: D-17 — the two checks merge into one command and one report,
  over keeping `format validate` and `aokf validate` side by side; two
  `Finding` types, two renderers, two `--json` shapes, two hooks and two
  file walks is how they drift, and `knowledge/schemas/` is already walked
  by both.
  Decision: D-18 — the merge happens above `aokf::validate`, which is left
  untouched, in a caller that runs both and concatenates findings, over
  putting the format checks inside `validate`; `validator_parity` compares
  that function's output verbatim against goldens captured from a reference
  that no longer exists, so new findings inside it break every golden with
  no way to regenerate them.
  Decision: D-20 — the verb is promoted to `superdev validate`, with `aokf
  validate` kept as a hidden alias and the hook marker unchanged, over
...
- Cases:
  ...
  - e2e: `superdev validate` with no arguments reports the canonical
    knowledge's concepts and the format's 61 files in one report, with one
    exit code — one command, one report, and the default scope is the
    knowledge plus the grammar's `roots` (FR-5, FR-6).
  - e2e: `knowledge/schemas/spec.md` appears once in that report, carrying
    both its AOKF and its format findings (FR-5).
  - e2e: `superdev validate --json` output parses and carries a single
    `findings` array in the shape aokf already emits (FR-5).
  - unit: the doc render output equals its captured golden — the command
    renders the grammar as prose, as the reference's `--doc` does (FR-9).
  - integration: a format error fails the run without changing what the
    AOKF checks reported (FR-8).
  - e2e: moving `.agents/format/grammar.yaml` aside leaves the command
    working and reporting the same findings — the binary validates from its
    embedded copy (FR-11).
  - unit: a test asserts the embedded grammar equals
...
- Change:
  ...
  2. Delete the reference — move the grammar to `.agents/format/grammar.yaml`,
     ...
     reverse in the sense that matters: the goldens from block 2 must already
     be committed, because this step ends the ability to make more.
  3. Say the command exists — add it to `knowledge/api-contracts.md` and to
     the command set, per D-8.
  Files: `crates/app/superdev/src/aokf_cli.rs` (the hook fires on knowledge
  or root); `.github/workflows/checks.yml`, `.github/PULL_REQUEST_TEMPLATE.md`
  and `CONTRIBUTING.md` (the renamed check);
  `knowledge/development-procedure.md`, `knowledge/definition-of-done.md`,
  `knowledge/error-handling.md` and `knowledge/software-components.md` (the
  renamed check and the widened hook); `package.json` (`check:aokf` renamed
  and pointed at the merged command); `.agents/format/grammar.yaml` (the
  grammar's home, moved from the script's directory);
  `scripts/superdev-format/` (deleted — script, grammar and meta-schema);
  `knowledge/development-commands.md` (the renamed check in the command
  set); `knowledge/api-contracts.md` (the command in the stable CLI
  surface).
  Decision: D-5 — the Node script is deleted last, over deleting it as soon
  as Rust compiles; it is the only thing that can produce a golden, so it
  outlives the code that replaces it.
  Decision: D-8 — the command is documented in `api-contracts` as stable,
  over leaving it undocumented until it settles; adding a verb is additive
  within a major version, and an undocumented verb is one nobody finds.
  Decision: D-11 — duplication stays a hard error, and the hook revalidates
...
- Done-check: every case across the plan passes on a clean checkout of the
  ...
  done; the changelog has an Unreleased entry for the promoted command; the
  pack question is filed as
  [issue-016][sokf:issue-016-sync-would-revert-the-schema-migration], since
  the live tree and `/pack/` diverge until it is answered; the
...
- Cases:
  ...
  - observation: `rg -n 'superdev-format' --hidden` returns nothing outside
    this plan and the changelog — `scripts/superdev-format/` no longer
    exists (FR-10).
