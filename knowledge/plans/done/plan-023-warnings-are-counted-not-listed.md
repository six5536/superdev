...
- Verification: a bare `superdev validate` on this repository lists no
...
- Verification: `superdev validate --json` carries `errors` and `warnings`
...
- Verification: on a repository carrying one error and one warning, the
