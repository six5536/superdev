...
- Verification: a fixture contract carrying
...
- Verification: a fixture schema declaring `content: include` passes a
...
- Verification: a fixture schema with `variant-key: kind` and a section
...
- Verification: the schema validates with all twelve examples; a fixture
...
- Verification: `normative_shapes` and the skill grammar pass on the
...
- Verification: both contracts validate under the contract schema; the
...
- Verification: the three contracts validate; the Settings table of
...
- Outcome: markers around the `pub` items `contract-007`, `contract-009`
  ...
  declaration.
- Verification: the three validate; `contract-010`'s Definition shows the
...
- Outcome: `contract-008` becomes `kind: format`. Its definition is the
  ...
  Definition names.
- Verification: `contract-008` validates; its Definition shows the token
...
- Verification: `ls knowledge/schemas/contract-*.md` lists nothing;
