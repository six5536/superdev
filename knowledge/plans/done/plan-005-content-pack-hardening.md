...
binary cannot read. The one unprompted network request carries a deadline
and never prompts, and a path pack records no digest.

The five decisions taken against the issues S014's acceptance left open
(S014 retired). Contract:
[C007][sokf:contract-007-interface-pack-resolution]. Decisions:
[ADR-012][sokf:adr-012-pack-source-schemes-are-allowlisted],
[ADR-013][sokf:adr-013-update-proves-a-pin-before-it-writes-it],
[ADR-014][sokf:adr-014-a-symlink-in-a-pack-is-refused],
...
[P003][sokf:plan-003-content-packs], which is done and whose spec is
accepted. P003's *Not scheduled* list is what became this one, less
[I003][sokf:issue-003-a-local-pack-cannot-remove-what-it-dropped], closed
`wontfix`: a path pack keeps layering, and the rebuild a pack developer
needs anyway is the answer.

Blocks are ordered by dependency, then risk. Block 1 is the half of the
security issue still open and carries the risk of refusing a spelling
somebody legitimately uses, so it goes first. Blocks 2 and 3 narrow what a
pack may contain, which is the other way this work can break something that
works today. Block 4 changes the seam every spawn in the codebase shares and
observably does nothing, which is what makes it safe to put in the middle.
...
- contract-007-interface-pack-resolution: pack resolution narrows.
  `P_transport-allowlisted` — `resolve` admits https, ssh and file and
  refuses every other transport, naming the source. `P_pack-carries-no-symlink`
  and `P_symlink-fails-resolution` — a symlink or a submodule anywhere under
  the pack fails the run, naming the path, both on a path pack and on a
...
- Change: add `SUPPORTED_SCHEMES` to `pack/source.rs`. `PackSource::parse`
  ...
  `protocol.{git,http,ext}.allow=never`. Correct `README.md`, which today
  says every git URL other than the shorthand "is handed to `git` as
  written" — [P003][sokf:plan-003-content-packs]'s block 18 wrote that
  knowing this block would narrow it, and named this block as the one to fix
...
- Cases:
  - unit: `git://`, `http://` and `ext::` sources are refused at parse,
    ...
    transport allowlist.
  - unit: `github:o/r`, `gitlab:o/r`, `https://`, `ssh://`, the scp form, a
    bare ssh alias and `file://` still parse — checks that no supported
    spelling is lost.
  - integration: the override set read off a real call carries all seven
    `protocol.*` settings — checks that the git-side half is present.
  - integration: with a global config rewriting `https://…` into `ext::touch
    <path>`, a source `parse` approves still runs no command. This is the
    case that matters: it proves the override half is not decoration, since
    `parse` cannot see a rewrite and only the named refusal outranks a user
    config saying `protocol.ext.allow = always`
...
- Cases:
  ...
  - unit: a fetched pack read back from the cache fails the same way —
    checks that the rule covers both read paths.
  - regression: a link to a secret outside the pack still writes nothing
    (I008) — checks that the earlier hole stays closed.
...
- Cases:
  - integration: a fixture repository whose pack holds a symlink is refused
    ...
    filesystem, decides.
  - integration: the same repository is refused when the working tree holds
    the plain file a `core.symlinks=false` checkout produces. This case is
    the point of the block and is reproducible on Linux, so it belongs in
    the ordinary suite rather than the Windows job: set
    `core.symlinks=false` in the fixture repository's own config and force a
    re-checkout — passing `-c core.symlinks=false` to `clone` does not work,
    because clone probes the filesystem and writes its own value.
  - integration: a submodule under the pack is refused the same way —
    checks mode `160000`.
...
- Change: `update_pins` takes `&Lock` and resolves the entry it is about to
  ...
  it did on every surface that states it — clap's `long_about` in `main.rs`,
...
- Cases:
  ...
  - integration: a readable release still moves the pin, and a second
    unresolvable entry does not hold back a move that is fine.
  - integration: an unreachable source behaves exactly as it does today.
  - regression: P003's cases 16, 17, 20 and 22 cover `update`'s pin
    movement and must still pass. Cases 17 and 20 passed untouched. Cases 16
    and 22 did not pass unchanged, as this block first expected: the probe
    is a real resolution, so a fake runner answering only `ls-remote` can no
    longer satisfy the move they assert. They now start from a repo whose
