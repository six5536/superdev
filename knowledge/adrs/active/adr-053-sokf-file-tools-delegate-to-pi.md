...
These snapshots will preserve the `clean`, `pending-auto(0..2)`, `manual`, and `pending-manual` follow-up states on the active session branch. Only valid final validation resets the sequence. `degraded` is an explicit repository-scoped in-memory mode, not a durable snapshot variant. It carries the intended durable state and `validationDue`, suppresses triggered follow-ups, and ends only after the exact intended snapshot appends.

A failed append after apply preserves file-tool success, merges diagnostics, sets `validationDue`, and causes one validation at the next eligible `turn_end`. A failed append before a triggered correction preserves the prior count, clears `validationDue`, and suppresses that message. A failed append while entering or returning to `manual` retains intended `manual`, clears `validationDue`, and replaces the nominal message with non-triggering snapshot-failure guidance. A failed clean append retains intended `clean`, clears `validationDue`, and reports that the durable reset failed. A later applied mutation in degraded mode merges diagnostics, sets `validationDue`, and preserves any unresolved count while retrying the exact intended snapshot.
...
- Negative: snapshot persistence failure suspends triggered correction turns until the adapter appends the exact intended state.
