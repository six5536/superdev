...
- `P_pi-orchestrates` [ubiquitous] Pi SHALL orchestrate interaction and isolated roles through explicit `scope`, `build`, and `accept` skills backed by one typed phase tool while withholding direct shell and Git access from every child, giving BUILD only its Rust-owned checkpoint, attempt, correction-checkpoint, and status operations through the parent-pinned executable and digest, and blocking isolated invocations of authoritative workflow operations.
- `P_extension-skills` [ubiquitous] The Superdev Pi extension SHALL contribute its `scope`, `build`, and `accept` skills through extension-local resource discovery.
- `P_skill-cold-start` [event] WHEN Pi invokes a workflow skill without prior workflow conversation, the skill SHALL reconstruct canonical phase, workflow identity, issue, plan, terminology, user argument meaning, and valid typed outcomes before taking an authoritative action.
- `P_skill-intake` [event] WHEN an explicit workflow skill receives free-form intent, the LLM SHALL interpret it semantically, search canonical knowledge, discuss one recommended issue and plan selection with the human, author either missing record after confirmation, and pass exact independently selected identities to the typed phase tool.
- `P_internal-workflow-tools` [ubiquitous] Low-level ownership, revision, question-queue, and isolated phase-role operations SHALL remain unavailable to the main model; skills interact through human-level typed phase operations.
...
- `P_output-bounded` [ubiquitous] Pi SHALL incrementally parse isolated events, keep model-visible output within configured byte and line limits, persist bounded typed results and diagnostics in owner-only temporary artifacts, and keep raw events, reasoning, prompts, and tool payloads out of model context and session history.
- `P_isolated-trace` [ubiquitous] Pi SHALL retain each isolated child's raw JSON event stream and stderr in owner-only temporary artifacts up to the configured artifact-byte limit and report each exact path plus retained and discarded byte counts in every diagnostic.
...
- `P_scope-bootstrap` [event] WHEN the SCOPE skill has authored a confirmed issue and initial plan, the service SHALL validate both records, require the plan's sole `implements` relationship to target the issue, permit an independently numbered plan identity, derive the reserved work branch from the issue identity, reject unrelated working-tree changes, and atomically publish the exact authored records before entering SCOPE.
- `P_scope-attempt-baseline` [event] WHEN Pi starts or retries SCOPE authoring, the service SHALL compare-and-swap an expected current work-branch tip as that attempt's product baseline only while every uncommitted path remains under `knowledge/`.
- `P_scope-publication` [event] WHEN isolated SCOPE work changes canonical knowledge, a SCOPE checkpoint SHALL reject every product change after the service-owned SCOPE attempt baseline, validate the complete knowledge snapshot, publish one knowledge-only commit before isolated review, and bind subsequent scope-review evidence to the canonical plan revision and immutable checkpoint commit reviewed by Pi.
...
- `P_final-correction-service` [event] WHEN immutable final review reports findings, the service SHALL schedule a revision-bound correction, invalidate that candidate's final evidence, require a successful correction checkpoint that reruns approved Verification commands and commits only within SCOPE-approved block Areas, and count the cycle only after a complete valid re-review.
