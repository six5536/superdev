...
Observed on the dogfood repository, where the pack source and the base are
meant to be the same content.
...
Instead, step 2 prints `knowledge (aokf): ok` and writes nothing.
...
No fix is proposed; the workaround is the rebuild.

- Fix: none. See the Resolution below. The two candidates were an
...
- Workaround: rebuild the binary after deleting or renaming under `pack/`;
  ...
  snapshot.
- Regression risk: `pack/resolve.rs`'s base decision and the orphan pass. A
  test would drop an item from a local pack and assert the live copy goes, or
  that the run says why it cannot.
...
above is corrected accordingly: `.claude/skills/<name>/` is an owned file, so
