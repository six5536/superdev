---
type: Issue
id: issue-043-the-cli-contracts-json-keys-are-bound-by-no-test
...
The gap is a missing binding, not a runtime failure.
...
Instead, the two lists disagreed and every test passed:
...
The fix binds the block by running the binary.
...
- Regression risk: the binding would read the same contract block the
  CLI drift test reads and the same binary the exit-code probes run, so
  it adds no new mechanism; a key added to the report without its
  declaration is what it would catch.
