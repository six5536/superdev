---
type: Issue
id: issue-048-no-step-asks-whether-a-contract-is-bound
...
- When a feature is accepted, the step reports each contract the
  feature touched that no test and no generated artifact appears to
  bind.
- For each contract it reports, the step names what it searched and
  what it did not find, so a reader confirms or dismisses the report
  without repeating the search.
- The step presents the report as a judgement that blocks nothing, and
  does not record it as a validator finding.
- While a contract element is marked pending, the step counts the
  reverse binding ADR-038 requires as that element's binding, rather
  than reporting it as unbound.
- The step ships in the pack, so a project adopting superdev inherits it
  with the schemas that state the obligation.
- If the feature touched no contract, the step says so and reports
  nothing further, so an empty report is distinguishable from a step
  that did not run.
- If the step cannot read the project's tests, it reports what it could
  not read and does not report the contracts it could not judge as
  unbound.

## Scope

The feature adds the step and its report, and no gate.
