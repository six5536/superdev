---
type: Issue
id: issue-051-pin-node-in-the-managed-repo
...
The pin reaches one managed key and every managed repository's mise env.
...
- `superdev sync` writes a `node` entry beside `codegraph` in
  `.mise.toml` and the lock claims it.
- `mise install` in a managed repository with no node on the host
  installs `codegraph` through the `npm:` backend.
- `codegraph init` runs from the installed shim in that repository.
