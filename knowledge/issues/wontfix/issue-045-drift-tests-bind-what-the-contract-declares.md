---
type: Issue
id: issue-045-drift-tests-bind-what-the-contract-declares
...
links:
  - rel: references
    ...
    note: The reframe that settled this — the copies the drift tests compared no longer exist.
  - rel: references
...
framing
[I036][sokf:issue-036-validate-prints-warnings-by-default].
...
- When `superdev validate --json` emits a top-level key that
  `contract-002`'s `json` block does not declare, a test fails naming
  that key and stating that the binary emits it undeclared.
- When `contract-002`'s `json` block declares a top-level key that
  `superdev validate --json` does not emit, a test fails naming that key
  and stating that the contract declares it unemitted.
- The two comparisons above cover the `repaired` key, which requires
  exercising `--fix`.
- When a command's flags, arguments or exit map differ between the
  binary and `contract-002`, the test fails naming the differing element
  and stating which of the two carries it.
- When a tool's arguments differ between the served MCP surface and
  `contract-003`, the test fails naming the differing argument and
  stating which of the two carries it.
- When a drift test reports a difference without stating the direction
  the difference runs in, the check that binds drift-test reporting
  fails.
- The tests compare every element `contract.rs` and `mcp.rs` compare
  today, so replacing a whole-struct equality with per-element
  comparisons loses no coverage.

## Scope

The feature binds the `json` block and names the direction of every
difference inside a bound element.
...
first found this repository's drift tests to be defects rather than a
feature, then found the copies they compared to be the defect. Under
I049 a contract's definition is materialised from source and cannot
drift, the four copy-comparing tests are deleted, and the seven
criteria above have nothing left to bind. What survives of them is
behaviour — the exit codes — which `contract_exit_codes.rs` already
tests.
