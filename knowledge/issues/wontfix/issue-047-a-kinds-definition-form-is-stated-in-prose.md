---
type: Issue
id: issue-047-a-kinds-definition-form-is-stated-in-prose
...
- The validator accepts any fence tag as a contract-kind schema's
  declared definition form, and parses a block's contents only where
  that form is `yaml` or `json`.
- When a contract's definition section carries no fenced block tagged
  with a form its kind declares, the validator reports an error naming
  the declared form and the tags the section carries.
- If a kind declares a definition form the validator does not parse,
  the validator reports no finding about that block's contents.
- If a kind declares block keys or block entry keys alongside a
  definition form the validator does not parse, the validator reports an
  error on the schema, naming the keys that bind nothing.
- Every contract-kind schema whose definition section takes a fenced
  block declares the fence tags that section accepts.
- Each contract-kind schema whose declared form the validator does not
  parse states that a drift test is what binds that section's block.
- Each kind whose form the validator does not parse gives the reason its
  ecosystem needs that form.
- Every contract on file passes validation.
- If a contract on file carries a fence tag its kind's declared forms do
  not admit, the change either admits that tag or changes the contract,
  and records which, so no such contract is made to pass by widening
  the declaration unexamined.

## Scope

The feature widens the form declaration and adds the tag check; it
changes no kind's form.
...
- Out: whether an individual contract names the test that binds it,
  which is
  [I048][sokf:issue-048-no-step-asks-whether-a-contract-is-bound]'s
  to weigh.
...
while framing
[I045][sokf:issue-045-drift-tests-bind-what-the-contract-declares],
