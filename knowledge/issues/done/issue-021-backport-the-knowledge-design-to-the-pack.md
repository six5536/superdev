---
type: Issue
id: issue-021-backport-the-knowledge-design-to-the-pack
...
links:
  - rel: relates-to
    to: issue-016-sync-would-revert-the-schema-migration
  - rel: relates-to
    to: issue-020-the-schemas-do-not-ship
---
...
[I016][sokf:issue-016-sync-would-revert-the-schema-migration] asked for:
something in the tree that says the drift is known and owned.

## Context

The surfaces the backport reaches, with the counts that bound them:
...
- `knowledge/schemas/` ships nowhere: the pack carries the 56 templates
  ...
  is [I020][sokf:issue-020-the-schemas-do-not-ship].
...
- P011's share, the lifecycle filing: the pack's `knowledge/concepts/`
  scaffold writes no state folders and its documents carry `status` and
  triage tags rather than a `lifecycle` key, so a freshly seeded repo
  fails the filing gate until `superdev validate --fix` runs; the 8 pack
  skills and 8 pack templates still name `knowledge/{issues,plans,specs,
  decisions}/` paths the live tree no longer uses; and the two templates
  saying "numbered after the highest in this directory" predate numbering
  across a kind's folders.
- P012's share, the contract-driven workflow: the workflow and support
  skills' pack copies were mirrored in step (spec and verify deleted,
  interface-design renamed contract-design) and `pack/agents/process.md`
  with them, but `pack/sokf/agents/SPEC.md` still glosses `implements`
  against a spec, `pack/sokf/agents/sokf.md` says nothing of the
  `lifecycle` filter, and the pack's templates still mint spec and
  per-feature interface-contract documents for kinds the live tree
  retired.
...
Done in one pass with `/pack-backport` (commit `d07365a`). Every
definition-of-done item holds: `status --drift` exits 0, a fresh
`superdev init` validates PASS, the manifest declares the version the
binary enforces, and I016 and I020 are settled. The AOKF name survives
under `pack/` only where the SOKF changelog records the rename — a
historical record, not operative vocabulary.
...
[I005][sokf:issue-005-a-backport-leaves-the-lock-stale]'s lock
reconciliation are the mechanics; the size is the reason it waits for one
pass rather than five.

Done in one pass with `/pack-backport`. Every definition-of-done item
holds: `status --drift` exits 0, a fresh `superdev init` validates PASS,
the manifest declares the version the binary enforces, and I016 and I020
are settled. The AOKF name survives under `pack/` only where the SOKF
changelog records the rename — a historical record, not operative
vocabulary.
