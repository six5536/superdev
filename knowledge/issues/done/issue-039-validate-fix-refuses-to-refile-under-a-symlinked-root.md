---
type: Issue
id: issue-039-validate-fix-refuses-to-refile-under-a-symlinked-root
...
Found at 0.2.0, `main` at 19ac275, and every commit back to `d07365a`,
which introduced the guard. The platform is macOS (`/var` and `/tmp`
are symlinks to `/private/...`), and any platform where the knowledge
...
Instead, the move fails and the error names the destination as outside
the knowledge:
...
called `Result::unwrap()` on an `Err` value: Io {
  path: "/var/folders/df/.../T/.tmpUQULvL/knowledge/issues/open/issue-001-a.md",
...
The root cause is in `move_within` at
`crates/lib/superdev-core/src/validate/fix.rs:292`, which checks both
ends of the rename:
...
The fix is in the guard's resolution of a path that does not exist yet.
...
Regression risk: `move_within` is the only mover in the fix pass, so
every lifecycle refile goes through it; the existing lifecycle test
catches a recurrence on macOS, and a symlinked-root test would catch
one everywhere.
