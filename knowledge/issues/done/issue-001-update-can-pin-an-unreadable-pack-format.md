...
Found at 0.2.0, slice 11 of P003 (`4ed647f`), on every platform. Not
reproducible against a real release yet; the mechanism is:
...
Instead, step 2 writes `rev = "assets-v2.0.0"` to `.superdev/config.toml`
and saves it, then the `sync` that follows fails:
...
The root cause: `crates/lib/superdev-core/src/pack/pin.rs:95` chooses the
target on the tag alone — nothing in an `assets-vX.Y.Z` tag says what format
it holds. `crates/app/superdev/src/manage.rs:364` then calls `manifest.save`
before `sync` runs, so the unreadable pin is persisted before anything
validates it, and the never-backwards rule (deliberate, and right for its own
purpose) makes the state absorbing.
...
- Regression risk: `pack/pin.rs`, `pack/manifest.rs`'s `SUPPORTED_FORMATS`
  gate, and `manage.rs`'s update path. A test would cut a fixture repo tagged
  with a format the binary refuses and assert the pin does not move and the
  run still succeeds.
...
[C007][sokf:contract-007-interface-pack-resolution].
