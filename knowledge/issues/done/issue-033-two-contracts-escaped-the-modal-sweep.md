---
type: Issue
id: issue-033-two-contracts-escaped-the-modal-sweep
...
links:
  - rel: references
    to: issue-029-contract-design-writes-verbose-prose
...
[I029][sokf:issue-029-contract-design-writes-verbose-prose]'s
criterion 1 every normative statement uses a modal verb, so criteria 1
and 5 fail on the feature head.

## Context

Found on the feature/contract-design-review head on 2026-09-01, on any
platform; the defect is contract prose.

- Run `grep -c -E 'MUST|SHALL|SHOULD|MAY ' ` over the nine active
  contracts.
- Observe 002 and 003 at zero while the other seven carry keywords.
...
Instead, 002 and 003 carry zero RFC 2119 keywords; promises are stated
as bare declaratives.

The root cause: the sweep read 002 and 003 as "dense but sound" and
trimmed reasoning clauses only; the feature-wide review's census
flagged 004 and 008 — the two untouched files — and the
touched-but-unmodalised pair escaped both passes.

## Scope

The fix is one of the two remedies, chosen at framing.
...
Regression risk: contract prose only; `superdev validate` pins the
documents' shape and the goldens pin nothing here. A standard
refinement touches the fragment, which rematerializes into all 15
schemas on the next `--fix`.
