---
type: Issue
id: issue-037-a-contracts-behaviour-is-not-written-as-ears
...
The feature is done when the schemas, the validator and the records
meet these expectations:

- The contract schema declares Behaviour and Stability as bullet lists
  whose every item carries a key and one of the six EARS tags —
  `[ubiquitous]`, `[event]`, `[state]`, `[conditional]`, `[optional]`,
  `[complex]` — and prose and tables beside the list remain permitted
  in both sections.
- When an item under Behaviour or Stability, at any heading depth
  within the section, carries no EARS tag or no key, the validator
  reports an error naming the section and the item.
- When an item's key is not `P_` followed by lowercase words joined by
  hyphens, the validator reports an error naming the item and the key.
- When two items in one contract carry the same key, the validator
  reports an error naming the key and both items.
- When a modal verb — `SHALL`, `SHOULD`, `MAY`, `MUST`, `REQUIRED`,
  `RECOMMENDED` or `OPTIONAL`, with or without `NOT` — appears in
  Behaviour or Stability outside an item, the validator reports an
  error naming the section and the line.
- When an item under Behaviour or Stability carries `MUST`, `MUST NOT`,
  `REQUIRED`, `RECOMMENDED` or `OPTIONAL`, the validator reports an
  error naming the item, the matched verb and the schema rule.
- When an item under Behaviour or Stability carries more than one modal
  verb — `NOT` belongs to its verb — or a tag and no modal verb, the
  validator reports an error naming the item.
- When an item under Behaviour or Stability opens with `TBD`, the
  validator reports an error naming the item; the criterion form's
  `TBD` is not admitted in a contract.
- An item carrying `PENDING` beside its modal verb, as ADR-044 places
  it, passes the checks above.
- A finding from criteria 2 to 8 is an error that fails `superdev
  validate`, and `superdev validate --fix` does not rewrite a statement
  or supply a key.
- The contract schema states the citation form — the bare key where the
  contract is the subject, the contract's id followed by the key
  elsewhere — and a search for the key finds the promise and every
  citation.
- The contract schema's twelve examples each carry Behaviour and
  Stability in the form, and pass the schema's own check.
- Every active contract's Behaviour and Stability conform, with each
  former sentence's requirements as one keyed item each and no promise
  dropped, and `superdev validate` passes on the live tree.
- The grammar that governs schema files accepts the contract schema as
  changed, and contract-010 carries any row the change adds.
- No skill changes for the form; the contract schema alone carries it.
- The glossary defines the form of a contract promise and its key, and
  the changelog carries the change under Unreleased.
- The feature-request, bug-report and chore schemas declare a key on
  every item of the lists a plan case cites — `AC_` on Acceptance
  criteria; `RS_` on Steps to reproduce; `DD_` on Definition of done —
  of the form criterion 3 names with that prefix, unique within the
  issue, and a missing, malformed or duplicate key is an error as
  criteria 2 to 4 report one.
- A criterion's key precedes its EARS tag or its `TBD`, a repro step's
  or an expected behaviour's key stands alone, the list stays numbered,
  and each schema states the citation form of criterion 11.
- The feature-plan schema has a case name the keys of the criteria it
  covers.
- Every issue on file carries a key on each cited item — the slug
  `c<n>`, `n` the item's number, for an item on file at the sweep —
  every open plan's cases cite keys, and a settled plan or a test
  comment citing a number stands as a citation of the `c<n>` key.
- The three tracker schemas' and the feature-plan schema's examples
  carry the form and pass their own check, and `superdev validate`
  passes on the live tree.

## Scope

The work reaches the contract schema, the validator, the tracker
schemas and the records on file, and stops at the sentence beyond its
tag and verb.
...
- Out: acceptance criteria, which are EARS already and keep "THE SYSTEM";
  ...
  a plan case's keys exist in its issue, or that a test cites every
  key — testing a project's behaviour is a stated non-goal, and the key
  is what such a check would need if the non-goal were ever lifted; the
  grammar rules of `.agents/superdev.md`, whose RFC 2119 reference
  already treats `SHALL` as `MUST`.
