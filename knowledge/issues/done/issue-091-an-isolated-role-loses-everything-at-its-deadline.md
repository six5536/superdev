...
Done by the workflow redesign ([ADR-056][sokf:adr-056-scope-is-human-led-and-approval-is-checkout-local]),
which removes the condition rather than raising the limit.

There is no deadline-killed isolated role. BUILD and ACCEPT run in one
persistent worker session that keeps its identity across stages, pauses, and
restarts, and SCOPE runs in the controlling conversation with no child at all.

Completed work survives, by both designs the issue described. Each work block is
verified and committed on its own, and the durable record in
`.superdev/workflows/` holds the completed blocks, the stage to resume from,
unfinished work, and the candidate. A later run continues from those facts
rather than repeating them. Correction budgets are consumed durably, so a
reset, pause, or restart no longer refills them.

Stopping is bounded rather than immediate: the controller asks the worker to
stop, waits, and terminates its process tree only if it does not. An interrupted
stage is reported as interrupted, never as finished work, so the checkout is
inspected before continuing.

The session file stays local under the ignored `.superdev/cache/`, as recovery
and debugging data. It is never approval or progress authority, so persisting it
does not make a transcript into state.

Evidence: the worker runtime case in
`scripts/test/superdev-workflow-runtime.test.mjs`, which proves one session
across stages, restart reusing it, an interrupted stage reported as such, and
termination of an unresponsive tree; and
`scripts/test/superdev-workflow-execution.test.mjs` for block commits and
durable budgets.
