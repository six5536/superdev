---
type: Issue
id: issue-050-the-exit-code-probe-ends-a-live-unattended-run
...
links:
  - rel: references
    to: issue-026-rehearse-the-driver-on-a-real-feature
...
[I026][sokf:issue-026-rehearse-the-driver-on-a-real-feature]
...
Observed in this repository, on any platform, in
`crates/app/superdev/tests/contract_exit_codes.rs`, whose `run()` sets
`current_dir(REPO_ROOT)` for every probe.
...
Instead, the run state is removed, and the driver's next `run advance`
reports `no run is active`.

The root cause: `run()` uses one working directory for every probe
because most probes need the repository — `validate` reads the
knowledge. The `run` verbs do not, and `end` writes.

## Scope

The fix is confined to the probe's working directory.
...
Regression risk: low; the probe list is the same; two probes move to a
scratch directory where `end` finds nothing and returns 0, and
`advance` finds nothing and returns 2, which are the codes the contract
declares.
