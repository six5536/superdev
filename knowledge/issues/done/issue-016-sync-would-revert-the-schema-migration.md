...
Found on superdev 0.2.0, branch `feature/content-packs`, on any platform;
the drift is content, not machine state.
...
Instead, `status --drift` lists 65 changes and exits 1, and every one of them
is a revert:
...
the debt say it in a Definition-of-done bullet nobody reads while looking at
a failing check.
...
- Backport the migration into `/pack/`: the 39 schemas replacing the 41
  ...
  [I021][sokf:issue-021-backport-the-knowledge-design-to-the-pack], which
  also records that the divergence is deliberate while the knowledge design is
  still moving — the answer this issue asked for under Behaviour.
...
- `.agents/process.md` still tells the agent that document skeletons live in
  ...
  the rest.
- Regression risk: the backport is the fix and it is also the risk: `sync`
  writes over live files, so a backport that lands the pack content without
  first reconciling the lock hashes reports every touched file as
  user-edited and backs it up — the failure
  [I005][sokf:issue-005-a-backport-leaves-the-lock-stale] already recorded.
  The `pack-backport` skill exists for this, and `superdev status --drift`
  returning 0 is what proves the job finished.

## Resolution

Resolved by the
[I021][sokf:issue-021-backport-the-knowledge-design-to-the-pack] backport,
done in one pass with `/pack-backport` (commit `d07365a`): the pack carries
what the live tree carries, `superdev status --drift` exits 0, and `sync` is
safe to run in this repository.

## Comments

Resolved by the I021 backport: the pack carries what the live tree
carries, `superdev status --drift` exits 0, and `sync` is safe to run in
this repository.
