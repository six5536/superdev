...
Found on superdev 0.2.0, after P008, on any platform; the gap is in what the
content pack carries.
...
Instead, there is no finding. Before this issue's companion fix the run said
nothing at all about it; it now says:
...
The root cause: `crates/lib/superdev-core/src/content/layout.rs` classifies
pack content by position, and has rules for `knowledge/concepts/`,
`knowledge/skills/` and `knowledge/templates/` — none for
`knowledge/schemas/`. Nothing under `pack/knowledge/schemas/` would be an
item even if the files were put there, and they are not: this repository's
41 schemas are its own, tracked outside the pack.
...
- Regression risk: adding 41 files to the pack widens the drift
  `status --drift` reports until a repo syncs, and I016 already reports 65.
  The layout rules are covered by `content::layout` unit tests, which a new
  position must extend rather than replace —
  `paths_matching_no_rule_are_not_items` is the test that would otherwise
  quietly accept a typo in the new rule.

## Resolution

Resolved by the I021 backport (commit `d07365a`): `classify` has the
`["knowledge", "schemas", name]` rule, the kind is `DocSchema`, the SOKF
component writes the schemas as owned files — `sync` repairs a user's
edit — and the pack ships all 55 at `pack/knowledge/schemas/`. A fresh
`superdev init` seeds them beside the concepts they govern.

## Comments

Resolved by the I021 backport: `classify` has the
`["knowledge", "schemas", name]` rule, the kind is `DocSchema`, the SOKF
component writes the schemas as owned files — `sync` repairs a user's
edit — and the pack ships all 55 at `pack/knowledge/schemas/`. A fresh
`superdev init` seeds them beside the concepts they govern.
