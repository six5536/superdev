...
A recommendation now identifies its choice rather than describing it. Every
choice carries a stable `{ id, label }`, and `recommendation` is
`{ choiceId, reason }` naming one of them; `validateQuestion` refuses a
recommendation that names no supplied choice, so the two cannot drift apart.
The UI sorts the recommended choice first and appends `[Recommended]` to its
label.

Both constraints the issue named are kept. The marker is presentation only: the
selection is mapped back to the choice it came from and the recorded answer is
the plain label, so no marker travels into saved state. Control entries stay
after the answers, and a choice may not take a control's identity or label.

One surface now exists rather than two: `superdev_ask` and the SCOPE checklist
share `lib/questions.ts`. `intake.ts` and the separate review queue are gone
with the child roles.

Evidence: `scripts/test/fixtures/superdev-scope-questions.ts` and the real-Pi
journey in `scripts/test/superdev-workflow-scope.test.mjs`.
