...
Found at 0.2.0, P003 complete (`e1ac431`), on unix; Windows needs the
symlink privilege.
...
Instead, step 4 prints `SUPER-SECRET-KEY-abc123`. The repo now holds a real
file carrying the secret — confirmed, not inferred.

The root cause is at `crates/lib/superdev-core/src/pack/resolve.rs:349`:
...
- Regression risk: `pack/resolve.rs`'s walker, and any pack that legitimately
  ships a symlink — superdev's own `/pack/` does not. A test would put a
  symlinked file in a fixture pack and assert it is not among the resolved
  files.
...
[I009][sokf:issue-009-a-skipped-symlink-says-nothing]: it is silent, so a pack that
