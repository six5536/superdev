...
Pi 0.85.1 exposes the executable reference through `createReadTool()`, `createEditTool()`, `createWriteTool()`, `ReadToolDetails`, `EditToolDetails`, diff helpers, inherited renderers, argument preparation, and `withFileMutationQueue()`. Pi scopes built-in tools and sessions to the active working directory, recognizes a linked worktree's `.git` pointer file, and keeps that worktree distinct from the main checkout. The SOKF MCP contract is unreleased and already permits tool and schema changes without a deprecation path, so preserving its current mixed semantic-and-file `sokf_read` behavior is not a requirement.
...
- In a linked Git worktree, the entire SOKF adapter uses the canonical active-worktree root rather than Git's shared common directory or main checkout. Source resolution, semantic retrieval, search, graph traversal, MCP process and index activity, mutations, repair, refiling, validation, and persisted follow-up state remain isolated to that worktree from both its root and nested working directories. Another checkout is outside the active worktree's containment boundary.
