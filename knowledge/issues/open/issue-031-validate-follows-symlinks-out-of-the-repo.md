---
type: Issue
id: issue-031-validate-follows-symlinks-out-of-the-repo
...
platform is any; confirmed live on Linux with a directory symlink to
/etc. To reproduce:
...
Instead, the walk enters the symlink target and reads what it finds; an
unreadable subdirectory fails the run loudly, a readable one is checked
and quoted.

The cause: `walk` in `crates/lib/superdev-core/src/validate/mod.rs` uses
...
The walk and the named-file reads in the validator.
...
- Regression risk: the pack walk's symlink refusal (ADR-014) is separate
  code and stays. A confinement check must not break the legitimate case
  the repository itself uses: `/pack` reaching `superdev-core` by symlink
  inside the repo. The validator snapshot suites would catch a skipped
