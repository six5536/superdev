...
- When an `index.md` carries frontmatter, the validator reports it, since
  SPEC §9 says an index has none.
- When an index is not heading-grouped link lists in the house bullet
  style, the validator reports the offending line.

## Scope

A schema over the live indexes, blocked behind schema-to-document
validation.
