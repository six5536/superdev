---
type: Issue
id: issue-027-the-unattended-run-is-undocumented-for-users
...
[I024][sokf:issue-024-the-workflow-cannot-run-unattended]
shipped. The same gap for content packs was
[I006][sokf:issue-006-content-packs-are-undocumented-for-users].
...
- The README describes the unattended run: the driver skill, the run
  verbs, the Stop hook, the watchdog cap, and the do-nothing default
  without a run.
- Where the README's command tour lists verbs, it lists `superdev run`
  and `superdev hook run` with the same brevity as their neighbours.
