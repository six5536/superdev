...
- When an index entry's text differs from the linked concept's
  `description`, the validator warns, naming the index, the entry and
  the concept.
- The validator compares case-insensitively on the first character and
  ignores a trailing full stop, matching the house style.

## Scope

The comparison in `check_indexes` and the warning it emits.
