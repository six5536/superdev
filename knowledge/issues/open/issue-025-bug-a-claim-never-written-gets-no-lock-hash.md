...
- superdev at commit `b69176d`, branch `feature/workflow-autonomy`
- Linux devcontainer, dev shim (`scripts/superdev` → `cargo run`)
...
The lock reconcile that
[I005][sokf:issue-005-bug-a-backport-leaves-the-lock-stale] added
...
The I005 regression tests cover the refresh side; a new case wants a
claim satisfied on disk before its first sync. The orphan pass reads the
same entries, so a wrongly added hash would surface there.
