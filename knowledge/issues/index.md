...
* [Sync re-records a hash only for a file it writes, so backporting an edit leaves the lock stale](I005-a-backport-leaves-the-lock-stale.md) - after a live edit is mirrored into the pack, sync has nothing to write and never refreshes that file's recorded hash, so the next legitimate write reports it as a user-edited file and backs it up.
