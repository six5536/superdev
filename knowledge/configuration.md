...
([ADR-001][sokf:adr-001-packs-manifest-section]). Nothing about a repo
...
([ADR-004][sokf:adr-004-base-pack-identity]); every other entry sits
...
([ADR-012][sokf:adr-012-pack-source-schemes-are-allowlisted]). The scp
...
([ADR-011][sokf:adr-011-path-pack-identity-is-root-relative]). A directory
...
([ADR-009][sokf:adr-009-update-queries-default-source]). That is the one
...
([ADR-015][sokf:adr-015-the-spawn-seam-carries-a-deadline]). No git call
...
happens ([ADR-014][sokf:adr-014-a-symlink-in-a-pack-is-refused]). A pack
...
stays the repo's ([I005][sokf:issue-005-bug-a-backport-leaves-the-lock-stale]).
...
([ADR-016][sokf:adr-016-a-path-pack-records-no-digest]). The per-file hashes
...
bytes this machine does not have ([ADR-005][sokf:adr-005-pack-cache-and-fetch]):
...
- The local embedding model lives in the *user* cache, not the repo:
  ...
  repo on the machine; see [technology-stack][sokf:technology-stack] for the pin
  itself.

The capability set is in [architecture][sokf:architecture]; the file-ownership
