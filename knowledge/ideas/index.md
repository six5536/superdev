...
* [A blocked session shows what the child is doing][sokf:idea-017-a-blocked-session-shows-what-the-child-is-doing] - while an isolated role holds the session, show enough of its reasoning and actions for the user to judge whether it is on track, rather than a timer and a tool name.
