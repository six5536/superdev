...
description: What a deployer must supply to run the software — the settings, where they come from, which source wins, and the stability promise, in knowledge/contracts/public/.
---

# Configuration Contract Schema

Structural rules for one public configuration contract, filed at
`knowledge/contracts/public/contract-{nnn}-config-{slug}.md`. Everything a deployer or user supplies to
