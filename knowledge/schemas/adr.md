...
id: schema-adr
title: ADR Schema
description: Architecture decision records — context, the decision, options considered and consequences — filed in knowledge/decisions/.
---

# ADR Schema

Structural rules for architecture decision records filed at
`knowledge/decisions/adr-{nnn}-{slug}.md` and listed in
`knowledge/decisions/index.md`. A superseded ADR is never edited: the new
...
frontmatter:
  ...
  status:
    const: stable
    description: >
      The concept's own status. The decision's lifecycle — proposed,
      accepted, superseded, deprecated — rides in the body Status line,
      not here.
