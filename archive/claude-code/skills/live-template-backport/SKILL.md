...
<step name="DOCUMENT" task="Document: update the template's entry under Files in the `contract-008-format-template` contract, read with `sokf_read` (or add one); its name and description arrive in the Definition through `validate --fix`, and `CHANGELOG.md`" />
