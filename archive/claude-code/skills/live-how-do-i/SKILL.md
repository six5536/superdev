...
- An issue or an idea to record without scoping it: `/file` — a bug, a feature request, a chore or an idea, in the user's words; `/scope` takes the issue up.
...
- Unattended delivery: `/execute-plan` drives `/build` over the plan's blocks on the work's branch, deferring the questions only the user can answer; `/scope` stays interactive.
